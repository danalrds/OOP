...
{
	int pos = this->findByTitle_Presenter(s.getTitle());
	if (pos != -1)
...
int Repository::findByTitle_Presenter(const std::string& title)
{
	//cout << title << endl;
...
Tutorial Repository::find_Obj_ByTitle_Presenter(const std::string& title)
{
	std::vector<Tutorial> lista = this->getTutorials();
